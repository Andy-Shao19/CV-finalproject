...
class RevResNet(nn.Module):
    def __init__(self, nBlocks, nStrides, nChannels=None, in_channel=None, mult=4, hidden_dim=16, sp_steps=2, kernel=3):
        ...
        self.size_stack = len(nBlocks) # RRB+Squeeze组数的个数
        ...
        self.mid_channel_reduction = channel_reduction(nChannels[self.size_stack//2], hidden_dim // (mult**sp_steps) , sp_steps=sp_steps, kernel=kernel)
    ...
    def _forward(self, x):
        ...
        i = 1
        mid_x = x
        for block in self.stack: # 一层一层的传递正向的RRB
            # 我们需要在中间的Block中，插入一个cWCT
            if i == self.size_stack // 2:
                mid_x = x
                mid_x = merge(x[0], x[1])
                
            i += 1
            x = block.forward(x) # 使用每个不同的block的forward，返回结果进入下一个block， 好像每个block都是一样的

        x = merge(x[0], x[1])

        x = self.channel_reduction.forward(x)
        # print('mid_x', mid_x.size())
        # print('x', x.size())
        # mid_x = self.mid_channel_reduction.forward(mid_x)
