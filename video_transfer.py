...
parser.add_argument('--style', type=str, default='data/style/gqj01.jpg')
...
RevNetwork.load_state_dict(state_dict['state_dict'], strict=False)
...
for i in tqdm(range(len(frame_list)), desc="Video Processing"):
    ...
    with torch.no_grad():
        # Forward inference
        z_c, mid_z_c = RevNetwork(content, forward=True)
        z_s, mid_z_s = RevNetwork(style, forward=True)
        ...
        if args.alpha_c is not None and content_seg is None and style_seg is None:
            ...
            mid_z_cs = cwct.interpolation(mid_z_c, styl_feat_list=[mid_z_s], alpha_s_list=[1.0], alpha_c=args.alpha_c)
        else:
            z_cs = cwct.transfer(z_c, z_s, content_seg, style_seg)
            mid_z_cs = cwct.transfer(mid_z_c, mid_z_s, content_seg, style_seg)
        # Backward inference
        # mid_z_cs 是不是应该指针对一半的模型的反向啊，应该怎么做呢？
        # 啊，不对是z_cs + mid_z_cs 将效果加起来，然后经历一次反向过程。
        z_cs = mid_z_cs*0.05 + z_cs
